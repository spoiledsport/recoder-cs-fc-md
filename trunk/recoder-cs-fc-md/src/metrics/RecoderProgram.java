...
public class RecoderProgram {
	 ...
	 */
	public static class ConsoleReports
		...
		public void modelChanged(ChangeHistoryEvent changes) {
			TreeChangeList news = changes.getChanges();
	...
	public final static ConsoleReports CONSOLE_OUTPUT =
	...
	private static String[] collectJavaFiles(String dir) {
		...
		StringMutableList list = new StringArrayList();
		...
		try {
			...
			for (int i = 0; i < args.length; i += 1) {
				...
				if (a.endsWith(".CS")) {
					...
					if (sfr.getCompilationUnitsFromFiles(files).size()
						< files.length) {
						warn(
							"Could not load some files from "
								+ args[i]
								+ " - ignoring");
					}
					...
					if (sfr.getCompilationUnit(args[i]) == null) {
						warn(
							"Could not find class with logical name "
								+ args[i]
								+ " - ignoring argument");
					}
