...
/**
   A tree walker that allows to customize the traversal.
   The walker can report elements during ascending,
   and may skip the traversal of certain subtrees on demand.
   @author AL
*/
public class CustomTreeWalker extends TreeWalker {
    ...
    */
	...
	from recursion. Default is false (no report).
     */
    ...
    /**
       Returns true if the current element is left. This element will not
       be reported any longer. If return reporting is switched off,
       only terminal elements will be reported as returning.
    ...
    /**
       ...
       should be reported, or not. This setting takes effect beginning at
       the next call to {@link #next()}. It does not affect reports of 
       returning nodes.
    */
	...
	if (current instanceof NonTerminalProgramElement) {
		...
		ProgramElement[] newStack = 
		    new ProgramElement[Math.max(stack.length * 2, 
						count + s + 2)];
