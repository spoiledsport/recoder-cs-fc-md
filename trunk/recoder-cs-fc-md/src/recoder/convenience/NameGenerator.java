...
/**
   Generator for variable and method names creating an unbound series
   of names derived from short cut versions of the base name or by
   enumeration. The generator can be configured for short style (temporary
   variables) or long style (public names) naming schemes. The generator
   takes care that no Java keywords are generated.
...
public class NameGenerator {
    ...
    /**
       Create a generator for names based on the given name using 
       default style.
    ...
    /**
       Create a generator for names based on the given type name.
       Uses short style for primitive types, default style otherwise.
	...
	if (strategy == SHORT_STYLE) {
	    ...
	    c = 0;
	    for (int i = 0; i < len; i += 1) {
		for (int k = shortCuts[i].length - ((i == 0) ? 1 : 2); 
		     k >= 0; k -= 1) {
    ...
    /**
       Separates single words of a base string using the following rules:
       1. the first letter belongs to the first word
       2. the last letter belongs to the last word
       3. a capital letter with an adjacent lower case letter or underscore
          starts a new word
     */
	...
	for (i = 1; i < len - 1; i += 1) {
		...
		if (Character.isLowerCase(e) || e == '_' ||
	...
	for (int j = 0; j < a.length; j += 1) {
	    ...
	    NameGenerator nc = new NameGenerator(a[j], 0);
	    for (int i = 0; i < 10; i += 1) {
		System.out.print(nc.getNextCandidate() + " ");
	    }
	    System.out.println();
	}
    }
