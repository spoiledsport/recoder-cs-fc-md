...
/** This utility class provides queries, factory methods and transformations
    on abstract entities that do not fall into one of the abstraction
    categories (Types, Variables and so on).
    <P>
    One class of auxiliary methods deals with child-parent relations.
    There are queries to navigate trees upwards until a certain parent class
    is reached, most prominently {@link #getParentCompilationUnit}.
    A related query, {@link #contains} checks the syntactical containment
    relation (is-equals-or-ancestor-of).
    <P>
    Some convenience tranformations allow to remove, prepend or 
    append single childs.
    Since these transformations report their changes on this low level only,
    it is admissible to optimize change notifications by leaving the 
    change history object <CODE>null</CODE> and notifying a broader change 
    instead.
...
public class MiscKit {
	...
	/**
	    Factory method that creates a not yet collated reference
	    qualifier from a qualified name.  This can be a reference to a
	    field or parameter or variable, or a reference to a package,
	    an inner or outer or local type.
	@return a qualified reference with the given name; parent links of
	...
	*/
	public static UncollatedReferenceQualifier createUncollatedReferenceQualifier(
		...
		do {
			...
			result =
				factory.createUncollatedReferenceQualifier(
					result,
	...
	/**
	   Query checking if a given non terminal is identical to or an
	   ancestor of the given node.
	   If any of the elements is <CODE>null</CODE>, there is no
	   ancestor relation.
	   @param root a non terminal that might be the ancestor of the given 
	   element.
	   @param node a program element that might be a descendant of 
	   (or identical to) the given non terminal.
	...
	/**
	   Auxiliary transformation that sets the name of the specified element
	   to the specified string, creating a proper identifier.
	...
	/**
	   Transformation that renames a program element. If the new name
	   is different from the old one, a new Identifier is created and
	   ...
	   @return the new identifier if it has replaced the old one; the old
	   identifier otherwise.
	   @deprecated replaced by Transformation.replace
	 */
	public static Identifier rename(
		ChangeHistory ch,
		NamedProgramElement namedElement,
		String newName) {
		Debug.asserta(namedElement, newName);
	...
	/**
	   ...
	   contained within. Returns <CODE>null</CODE>, if the program element 
	   is not part of a type declaration, e.g. a CompilationUnit.
	   @param pe a program element.
	   @return the innermost type declaration containing pe, 
	   or <CODE>null</CODE>.
	 */
	...
	/**
	   ...
	   @param pe a program element.
	   @return the innermost member declaration containing pe, 
	   or <CODE>null</CODE>.
	 */
	...
	/**
	   ...
	   subtree; useful for debugging purposes. Returns the first program
	   element encountered that has an invalid parent link, or
	   <CODE>null</CODE> if every element is linked correctly.
	   @param root the root of the subtree to check (is not checked itself).
	   @return an descendant of root that has a bad parent link, or
	...
	/**
	   Transformation that removes a single child from its parent.
	   This method conveniently computes the position of the child
	   and handles the change history notification. The method can
	   also handle compilation units properly, but otherwise assumes
	   that the parent link is defined. This method is not very useful
	...
	public static void remove(ChangeHistory ch, ProgramElement child) {
		...
		if (child instanceof CompilationUnit) {
			...
			int oldIndex = parent.getIndexOfChild(child);
	...
	/**
	   Transformation that replaces a single child by a different
	   one.  This method performs the exchange and handles the change
	   history notification. The method can also handle compilation
	   units properly, but otherwise assumes that the parent link is
	   defined. This method is not very useful if the change history
	   is left undefined, but will work nethertheless.
	   @param ch the change history to notify (may be <CODE>null</CODE>).
	   @param child the child to remove from its parent.
	...
	*/
	public static void replace(
		...
		Debug.asserta(child, replacement);
	...
	/**
	   Transformation that appends or prepends the given child to the list 
	   in the given parent. No checks for redundancy or vadility are
	   performed.
	   @param ch the change history to notify (may be <CODE>null</CODE>).
	...
	private static void add(
		...
		Using child,
		boolean asHead) {
		Debug.asserta(parent, child);
		UsingMutableList list = parent.getUsings();
		if (list == null) {
			parent.setUsings(new UsingArrayList(child));
		} else {
			if (asHead) {
				list.insert(0, child);
			} else {
	...
	/**
	   Transformation that appends the given Using to the list in the 
	   given Compilation Unit. No checks for redundancy or vadility are
	   performed.
	   @param ch the change history to notify (may be <CODE>null</CODE>).
	   @param parent the parent containing a list of childs to append to.
	   @param child the child to be appended to the list in the parent.
	 */
	public static void append(ChangeHistory ch, CompilationUnit parent, Using child) {
		add(ch, parent, child, false);
	}

	/**
	   Transformation that prepends the given Using to the list in the 
	   given Compilation Unit. No checks for redundancy or vadility are
	   performed.
	   @param ch the change history to notify (may be <CODE>null</CODE>).
	 ...
	 */
	public static void prepend(ChangeHistory ch, CompilationUnit parent, Using child) {
	...
	private static void add(
		...
		if (list == null) {
			parent.setBody(new StatementArrayList(child));
		} else {
			if (asHead) {
				list.insert(0, child);
			} else {
	...
	/**
	   Transformation that appends the given Statement to the list in the 
	   given StatementBlock. No checks for redundancy or vadility are
	   performed.
	   @param ch the change history to notify (may be <CODE>null</CODE>).
	   @param parent the parent containing a list of childs to append to.
	   @param child the child to be appended to the list in the parent.
	...
	/**
	   Transformation that prepends the given Statement to the list in the 
	   given StatementBlock. No checks for redundancy or vadility are
	   performed.
	   @param ch the change history to notify (may be <CODE>null</CODE>).
	   @param parent the parent containing a list of childs to prepend.
	...
	/**
	   Adds object mappings from original to cloned versions for each
	   node in a tree to an existing map.
	 ...
	 */
	public static void mapClones(
		ProgramElement originalRoot,
		ProgramElement cloneRoot,
		MutableMap map) {
		Debug.asserta(originalRoot, cloneRoot, map);
		for (TreeWalker w1 = new TreeWalker(originalRoot), w2 = new TreeWalker(cloneRoot);
			w1.next() & w2.next();
			map.put(w1.getProgramElement(), w2.getProgramElement()));
	}

	/**
	   Returns the element in the cloned tree corresponding to the original
	   node in the original tree. This method will report the original element
	   if it does not occur within the first tree, or if the second tree
	   is smaller than the first one.
	   <P>
	   If there are many queries for clones, it is more efficient to
	   instantiate an own mapping using a hash table and the
	   {@link #mapClones} auxiliary.
	   @param original the element to find the corresponding clone for.
	   @param originalRoot the root of the original tree.
	   @param cloneRoot the root of the cloned tree.
	 */
	public static ProgramElement getClone(
		ProgramElement original,
		ProgramElement originalRoot,
		ProgramElement cloneRoot) {
		Debug.asserta(original, originalRoot, cloneRoot);
