...
public class NewArray extends TypeOperator
 implements Reference, ReferencePrefix {
    ...
    public NewArray(TypeReference arrayName, ExpressionMutableList dimExpr) {
    ...
    protected NewArray(NewArray proto) {
        ...
        if (proto.arrayInitializer != null) {
            arrayInitializer = (ArrayInitializer)proto.arrayInitializer.deepClone();
        }
    ...
    /**
     dim must be >= getDimensionLengths().size()
     If not, the dimensions are ignored during pretty print, but the model
