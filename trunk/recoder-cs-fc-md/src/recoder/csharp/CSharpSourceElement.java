...
import recoder.list.*;
import java.io.*;
import recoder.csharp.SourceElement.Position; // patch 4 stupid compilers

/**
 Top level implementation of a Java {@link SourceElement}.
 This class already knows its {@link ProgramFactory}.
 @author AL
 */

public abstract class CSharpSourceElement implements SourceElement, Cloneable, Serializable {

    /**
       The CSharpProgramFactory.
     */

    protected static CSharpProgramFactory factory =
        CSharpProgramFactory.getInstance();

    /**
       Position bit encoding.
       Internal format is start line: 16 | start column: 8 | 
       end line: 16 | end column: 8 |
       blank lines: 8 | blank columns: 8.
     */
    ...
    */
    ...
    protected CSharpSourceElement(CSharpSourceElement proto) {
        if (proto == null) {
    ...
    */
    ...
    /**
       Sets the start position of the primary token of this element.
       To set the start position of the syntactical first token,
       call the corresponding method of <CODE>getFirstElement()</CODE>.
    ...
    public final void setStartPosition(Position p) {
        if (p == Position.UNDEFINED) {
            ...
            int lc = (Math.min(p.getLine(), 0xFFFE) << 8) |
                Math.min(p.getColumn(), 0xFF);
    ...
    /**
       Sets the end position of the primary token of this element.
       To set the end position of the syntactical first token,
       call the corresponding method of <CODE>getLastElement()</CODE>.
    ...
    /**
       Sets the relative position (number of blank heading lines and 
       columns) of the primary token of this element.
       To set the relative position of the syntactical first token,
       call the corresponding method of <CODE>getFirstElement()</CODE>.
    ...
    public final void setRelativePosition(Position p) {
        if (p == Position.UNDEFINED) {
            ...
            int lc = (Math.min(p.getLine(), 0xFE) << 8) |
                Math.min(p.getColumn(), 0xFF);
            positionBits &= 0xFFFFFFFFFFFF0000L;
            positionBits |= (long)lc;
        }
    }
