package recodercs.list;
import recodercs.ModelElement;
import recodercs.csharp.ProgramElement;
import recodercs.csharp.declaration.MemberDeclaration;

/**
   List implementation based on the doubling array technique for MemberDeclaration
   elements. Tries to reduce memory usage.
   @since 0.01
   @author AL
*/
public class MemberDeclarationArrayList
     extends AbstractArrayList
  implements MemberDeclarationMutableList {

    /**
       Creates an empty list.
       @since 0.01
    */
    public MemberDeclarationArrayList() {
	super();
    }

    /**
       Creates an empty list that guarantees efficient addition of
       up to the given amount of elements.
       @since 0.01
    */
    public MemberDeclarationArrayList(int initialCapacity) {
	super(initialCapacity);
    }

    /**
       Create a list containing all elements of the given array.       
       @since 0.01
    */
    public MemberDeclarationArrayList(MemberDeclaration[] a) {
	super(a);
    }

    /**
       Creates a list with one element.
       @since 0.03
    */
    public MemberDeclarationArrayList(MemberDeclaration a) {
	super(a);
    }
    
    /**
       Create a new list as a deep clone of the given one.
    */
    protected MemberDeclarationArrayList(MemberDeclarationArrayList proto) {
	super(proto);
    }
    
    public Object deepClone() {
	return new MemberDeclarationArrayList(this);
    }

    public final void set(int index, MemberDeclaration element) {
	super.set(index, element);
    }

    public final void insert(int index, MemberDeclaration element) {
	super.insert(index, element);
    }

    public final void insert(int index, MemberDeclarationList list) {
	super.insert(index, list);
    }

    public final void add(MemberDeclaration element) {
	super.add(element);
    }

    public final void add(MemberDeclarationList list) {
	super.add(list);
    }    

    public final MemberDeclaration getMemberDeclaration(int index) {
	return (MemberDeclaration)super.get(index);
    }

    public final MemberDeclaration[] toMemberDeclarationArray() {
	MemberDeclaration[] result = new MemberDeclaration[size()];
	copyInto(result);
	return result;
    }


    public final ModelElement getModelElement(int index) {
	return (ModelElement)get(index);
    }

    public final ModelElement[] toModelElementArray() {
	ModelElement[] result = new ModelElement[size()];
	copyInto(result);
	return result;
    }


    public final Object getObject(int index) {
	return (Object)get(index);
    }

    public final Object[] toObjectArray() {
	Object[] result = new Object[size()];
	copyInto(result);
	return result;
    }


    public final ProgramElement getProgramElement(int index) {
	return (ProgramElement)get(index);
    }

    public final ProgramElement[] toProgramElementArray() {
	ProgramElement[] result = new ProgramElement[size()];
	copyInto(result);
	return result;
    }

}
