...
public class RecoderUtils {
	...
	if (pme instanceof Package) {
	    return (ni.getNamespace(pme.getFullName()) == pme);
	}
    ...
    // has no constructor declarations AND is valid.
		...
		return parent.getDeclaredTypes().indexOf(pme) >= 0 &&
		    isModelPart(sc, parent);
	    }
	}
    ...
    public static DeclaredTypeContainer getContainer(ProgramModelElement p) {
	DeclaredTypeContainer con = null;
	if (p instanceof Member) {
	    con = ((Member)p).getContainingClassType();
	}
	if ((con == null) && (p instanceof DeclaredTypeContainer)) {
	    con = ((DeclaredTypeContainer)p).getContainer();
	}
	return con;
    }
	...
	do {
		...
		return ((ClassType)p).getNamespace();
	    }
	    p = p.getASTParent();
	} while (p != null);
	return null;
    ...
    public static String getShortName(Namespace p) {
